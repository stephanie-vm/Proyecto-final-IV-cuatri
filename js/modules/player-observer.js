class PlayerObserver{
  ...
  suscribe(type, subscriberFn, subscriberctx){
    if(!this.subscribers[type]){
      this.subscribers[type] = []; // this creates an array with diferents types, first verify if the array exits with the condition
    }
    this.subscribers[type].push({fn: subscriberFn, subscriberctx, type}); // this adds the new suscriber from collection
  }
